import React, { useEffect } from 'react'
import ReactPlayer from 'react-player';
import { useState } from 'react';
import axios from 'axios';

export default function Clip({url}) {

    const [audioSrc, setAudioSrc] = useState(null);
    const [a, setA] = useState("");


    return (

        <div className=" px-4 py-4 ">

            <div className="">
            {/* <button className='border p-4 border-white' onClick={onbCLick}>Get audio</button> */}
                <ReactPlayer
                    url = {url}
                    controls
                    width="100%"
                    height="50px"
                />
                {/* <svg
                    className="text-rose-700 hover:text-rose-800 pr-3   "
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                >
                    <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={1}
                        d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"
                    />
                    <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        strokeWidth={1}
                        d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                    />


                </svg> */}


            </div>

            <div className="flex-auto flex-col justify-evenly ">
                <div class=" my-3">
                    <div class="flex justify-between text-sm text-grey-darker">
                    </div>
                    {/* <div class="mt-1">
                        <div class="h-3 w-auto bg-gray-400 overflow-hidden mb-1.5 rounded-full">
                            <div class="w-auto h-1 rounded-full relative">
                                <span class="w-1/2 relative h-4 bg-red-500 p-20 rounded-sm"></span>
                            </div>
                        </div>
                    </div> */}


                </div>

            </div>



        </div>




    )
}
